...
class CLEVRTREE():
    ...
    def __init__(self, batch_size=16,
        ...
        # self.image_files = pickle.load(open("data/filter_im_temp.p","rb"))
        # self.tree_files = pickle.load(open("data/filter_tree_temp.p","rb"))
    ...
    def read_first(self):
        images, _, _, _,_ = self.next_batch()
        self.index_ptr = 0
    ...
    def next_batch(self):
        ...
        filenames= []
        # print(self.batch_size, len(self) - self.index_ptr)
        for i in range(0, min(self.batch_size, len(self) - self.index_ptr)):
            ...
            with open(data_file['tree'][index], 'rb') as f:
                tree = pickle.load(f)
            filenames.append(data_file['tree'][index])
            # print(tree.function)
            # load image
        ...
        return tf.convert_to_tensor(images), trees, categories, refetch,filenames
    ...
    def get_all(self):
        ...
        return tf.convert_to_tensor(images), trees
...
if __name__ == '__main__':
    loader = CLEVRTREE(phase='train',base_dir='/Users/mihir/Documents/projects/ProbabilisticNeuralProgrammedNetwork_tf//data/CLEVR/CLEVR_64_MULTI_LARGE')
    for i in range(3):
